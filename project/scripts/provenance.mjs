...
export async function sourceFiles(root) {
  ...
  async function visit(path) {
    ...
    if (stat.isSymbolicLink())
      throw new Error("Symlinks cannot be saved: " + path);
    if (stat.isDirectory()) {
      for (const name of await readdir(join(root, path))) {
        if (name.startsWith(".") || name === "node_modules")
          throw new Error("Hidden files and dependencies cannot be saved");
...
export async function saveDiff(root) {
  ...
  if (
    !/^[a-f0-9]{40}$/.test(provenance.startingSha) ||
    baseline.startingSha !== provenance.startingSha
  )
    throw new Error("Template baseline and starting SHA do not match");
  const current = await sourceFiles(root);
  const temp = await mkdtemp(join(tmpdir(), "cobalt-dashboard-diff-"));
  try {
    git(temp, "init", "-q");
    await writeTree(temp, baseline.files);
    git(temp, "add", "-A");
    git(temp, "commit", "-qm", "Template baseline");
    for (const path of Object.keys(baseline.files))
      if (!(path in current)) await rm(join(temp, path), { force: true });
    await writeTree(temp, current);
    git(temp, "add", "-A");
    await writeFile(
      join(root, "changes.patch"),
      git(temp, "diff", "--cached", "--binary", "--full-index", "HEAD"),
    );
  } finally {
